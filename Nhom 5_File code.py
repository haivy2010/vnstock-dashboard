...
class ReportGenerator:
    ...
    def _add_header(self, pdf, period, start_date=None, end_date=None):
        ...
        from datetime import datetime, timedelta  # Import cần thiết
        ...
        # Tính ngày bắt đầu và kết thúc nếu không có giá trị được truyền
        if period == 'W':  # Past Week
            end_date = end_date or datetime.now()
            start_date = start_date or (end_date - timedelta(days=7))
        elif period == 'M':  # Past Month
            end_date = end_date or datetime.now()
            start_date = start_date or (end_date - timedelta(days=30))
        elif period == 'Y':  # Past Year
            end_date = end_date or datetime.now()
            start_date = start_date or (end_date - timedelta(days=365))
        elif period == 'Custom':  # Custom range
            if not start_date or not end_date:  # Nếu thiếu ngày, báo lỗi
                raise ValueError("Custom period requires both start_date and end_date.")
    
        # Tạo chuỗi thời gian
        date_range = f"Period: {period}:{start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
