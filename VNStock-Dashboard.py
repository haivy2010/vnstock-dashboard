...
class StockData:
    ...
    def get_available_symbols(self):
        """Get list of stock codes from CSV file in folder"""
        try:
    ...
    def load_stock_data(self, symbol):
        """Download stock data from CSV file"""
        try:
            file_path = os.path.join(self.folder_path, f"{symbol}.csv")
            df = pd.read_csv(file_path, parse_dates=['Date'])
            return df
...
class PDFReport:
    ...
    def generate_pdf_report(top_positive, top_negative, top_high, top_low, start_date, end_date):
        ...
        pdf.cell(200, 10, txt="Vietnam Stock Dashboard Report", ln=True, align='C')
        pdf.ln(10)
...
def main_dashboard():
    ...
    st.write("This is the main dashboard. Add your content here.")

    # Nút Export PDF
    if st.button("Export PDF Report"):
        st.experimental_set_query_params(page="export_pdf")

# Export PDF Report UI
def export_pdf_report_ui():
    st.title("Export PDF Report")

    stock_data = StockData(FOLDER_PATH)
    symbols = stock_data.get_available_symbols()
        ...
        if start_date > end_date:
            ...
            with st.spinner("Processing data..."):
                ...
                for symbol in symbols:
                    data = stock_data.load_stock_data(symbol)
                    if data is not None:
                        data = data[(data['Date'] >= pd.Timestamp(start_date)) & (data['Date'] <= pd.Timestamp(end_date))]
                        if not data.empty:
                            price_change = data['Price Close'].iloc[-1] - data['Price Close'].iloc[0]
                            high_price = data['Price High'].max()
                            low_price = data['Price Low'].min()
                            data_summary.append({
                                'Symbol': symbol,
                ...
                # Generate PDF
                pdf_path = PDFReport.generate_pdf_report(top_positive, top_negative, top_high, top_low, start_date, end_date)
