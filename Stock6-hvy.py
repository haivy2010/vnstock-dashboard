...
class TechnicalAnalysis:
    ...
    def calculate_indicators(data, rsi_thresholds=(30, 70)):
        ...
        try:
            ...
            # MA, MA 20, and MA 200
            df['MA'] = df['Price Close'].rolling(window=50, min_periods=1).mean()
            df['MA_20'] = df['Price Close'].rolling(window=20, min_periods=1).mean()
            df['MA_200'] = df['Price Close'].rolling(window=200, min_periods=1).mean()
...
# Add Moving Averages to Overlay
    def _add_overlay_indicators(self, fig, indicators, row):
        """Add overlay indicators to main chart"""
        if 'SMA 20' in indicators:
            fig.add_trace(
...
class ReportGenerator:
    ...
    def generate_performance_report(self, period='1mo', start_date=None, end_date=None, output_path='stock_report.pdf'):
        ...
        try:
            ...
            # Volume analysis
            volume_analysis = self._analyze_volume_trends(performance_data)
            self._add_volume_analysis(pdf, volume_analysis)
            
            # Volatility analysis
            volatility_analysis = self._calculate_volatility(performance_data)
            self._add_volatility_analysis(pdf, volatility_analysis)
            
            # Add charts
            self._add_performance_charts(pdf, performance_data)
            
            # Technical Analysis Summary for top performers
            self._add_technical_analysis_summary(pdf, gainers_losers['top_gainers'][:3])
    ...
    def _add_header(self, pdf, period, start_date, end_date):
        """Add report header with title and date information"""
        ...
        if period == 'Custom':
            ...
            date_range = f"Period: {period_text}"
    ...
    def _add_volume_analysis(self, pdf, volume_analysis):
        """Add volume analysis section to the report"""
        pdf.add_page()
        pdf.set_font('Arial', 'B', 14)
        pdf.cell(0, 10, 'Volume Analysis', ln=True)
        pdf.ln(5)
        
        # Add highest volume stocks table
        pdf.set_font('Arial', 'B', 12)
        pdf.cell(0, 10, 'Highest Trading Volume', ln=True)
        self._add_table(pdf, volume_analysis['highest_volume'],
                       ['Symbol', 'Avg Volume', 'Price Change (%)'],
                       [30, 40, 40])

    def _add_volatility_analysis(self, pdf, volatility_analysis):
        """Add volatility analysis section to the report"""
        pdf.add_page()
        pdf.set_font('Arial', 'B', 14)
        pdf.cell(0, 10, 'Volatility Analysis', ln=True)
        pdf.ln(5)
        
        # Add most volatile stocks table
        pdf.set_font('Arial', 'B', 12)
        pdf.cell(0, 10, 'Most Volatile Stocks', ln=True)
        self._add_table(pdf, volatility_analysis['most_volatile'],
                       ['Symbol', 'Volatility (%)', 'Price Change (%)'],
                       [30, 40, 40])
    ...
    def _add_performance_charts(self, pdf, performance_data):
        """Add performance visualization charts to the report"""
        pdf.add_page()
        pdf.set_font('Arial', 'B', 14)
        ...
        returns = [stock['price_change'] for stock in performance_data]
        sns.histplot(returns, bins=30)
        ...
        plt.savefig(temp_plot_path, format='png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # Add plot to PDF
        try:
            pdf.image(temp_plot_path, x=10, y=None, w=190)
        finally:
            # Clean up temporary file
            if os.path.exists(temp_plot_path):
                os.remove(temp_plot_path)

    def _add_technical_analysis_summary(self, pdf, top_performers):
        """Add technical analysis summary for top performing stocks"""
        pdf.add_page()
        pdf.set_font('Arial', 'B', 14)
        pdf.cell(0, 10, 'Technical Analysis Summary - Top Performers', ln=True)
        pdf.ln(5)
        
        for stock in top_performers:
            try:
                pdf.set_font('Arial', 'B', 12)
                pdf.cell(0, 10, f"Technical Analysis - {stock['symbol']}", ln=True)
                pdf.set_font('Arial', '', 10)
                
                # Check if data exists and is valid
                data = stock.get('data')
                if data is None or data.empty:
                    pdf.multi_cell(0, 10, "No data available for analysis")
                    pdf.ln(5)
                    continue
                
                # Calculate technical indicators
                ta_data = TechnicalAnalysis.calculate_indicators(data)
                if ta_data is None or ta_data.empty:
                    pdf.multi_cell(0, 10, "Unable to calculate technical indicators")
                    pdf.ln(5)
                    continue
                
                # Get latest data safely
                try:
                    latest_data = ta_data.iloc[-1]
                    
                    # Format metrics with error handling
                    rsi = latest_data.get('RSI_14', 'N/A')
                    rsi_str = f"{rsi:.2f}" if isinstance(rsi, (int, float)) else "N/A"
                    
                    macd = latest_data.get('MACD_12_26_9', 'N/A')
                    macd_str = f"{macd:.2f}" if isinstance(macd, (int, float)) else "N/A"
                    
                    signal = latest_data.get('MACDs_12_26_9', 'N/A')
                    signal_str = f"{signal:.2f}" if isinstance(signal, (int, float)) else "N/A"
                    
                    # Calculate volume trend safely
                    try:
                        recent_volumes = ta_data['Volume'].iloc[-5:]
                        if not recent_volumes.empty and recent_volumes.notna().any():
                            volume_trend = 'Increasing' if ta_data['Volume'].iloc[-1] > recent_volumes.mean() else 'Decreasing'
                        else:
                    ...
                    pdf.multi_cell(0, 10, f"Error analyzing latest data: {str(e)}")
                
                pdf.ln(5)
                
            except Exception as e:
                pdf.multi_cell(0, 10, f"Error processing {stock.get('symbol', 'unknown')}: {str(e)}")
                pdf.ln(5)
def main():
    ...
    def create_indicator_selection():
        ...
        overlay_indicators = st.sidebar.multiselect(
            'Price Chart Overlay',
            ['SMA 20', 'EMA 20', 'Bollinger Bands','MA','MA 20','MA 200']
        )
